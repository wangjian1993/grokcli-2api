import { api, download } from '@/utils/g2a/request'
...
export const getUsageSeries = (days: number) =>
  api(`/usage/series?days=${encodeURIComponent(days)}`)
...
/** 账号额度。cached=true 只读库内 last_quota；force/refresh 会 live 拉取。ids 限定当前页。 */
export const getAccountsQuota = (
  force = false,
  opts: { cached?: boolean; ids?: string[] } = {},
) => {
  const params = new URLSearchParams()
  if (force || opts.cached === false) {
  ...
  return api(`/accounts/quota${q ? `?${q}` : ''}`)
}
export const getAccountQuota = (id: string, refresh = false) =>
  api(
    `/accounts/${encodeURIComponent(id)}/quota${refresh ? '?refresh=1' : '?cached=1'}`,
  )
...
export const testRegProxy = (body: Record<string, unknown>) =>
  api('/accounts/register-email/test-proxy', { method: 'POST', body: JSON.stringify(body) })
export const stopRegBatch = (id: string) =>
  api(`/accounts/register-email/batches/${encodeURIComponent(id)}/stop`, {
    method: 'POST',
    body: '{}',
  })
export const stopRegSession = (id: string) =>
  api(`/accounts/register-email/sessions/${encodeURIComponent(id)}/stop`, {
    method: 'POST',
    body: '{}',
  })
export const resumeRegBatch = (id: string) =>
  api(`/accounts/register-email/batches/${encodeURIComponent(id)}/resume`, {
    method: 'POST',
    body: '{}',
  })
export const reclaimRegister = (body: Record<string, unknown> = {}) =>
  api('/accounts/register-email/reclaim', {
    method: 'POST',
    body: JSON.stringify(body),
  })
export const getRegAvailability = () =>
  api('/accounts/register-email/availability')
export const exportRegSSO = (body: Record<string, unknown> = {}) =>
  api('/accounts/register-email/export-sso', {
    method: 'POST',
    body: JSON.stringify(body),
  })

/** 运维：Token 续期 / 模型探测 / 自动补号 / 上游 */
export const getMaintainer = () => api('/maintainer')
export const runMaintainer = (force = true) =>
  api('/maintainer/run', { method: 'POST', body: JSON.stringify({ force }) })
export const getModelHealth = () => api('/model-health')
export const getUpstreamStatus = (force = false) =>
  api(`/upstream-status?force=${force ? 1 : 0}`)
export const getAutoReplenishStatus = () => api('/auto-replenish/status')
export const runAutoReplenish = () =>
  api('/auto-replenish/run', { method: 'POST', body: '{}' })
